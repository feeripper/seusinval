...
import { DOMAINS, DOMAIN_META, Domain, trendSeries } from '@/lib/presentation';
...
function TrendTooltip({ active, payload, label, projected }: { active?: boolean; payload?: TooltipPayload[]; label?: string | number; projected?: boolean }) {
  ...
  return (
    <div className="min-w-[190px] rounded-xl border border-n-200 bg-n-0 p-3 shadow-[var(--shadow-2)]">
...
export function TrendChart({ rows, highlight }: { rows: Indicator[]; highlight?: Domain }) {
  ...
  const visibleData = mode === 'scenario' ? data : data.filter(p => !p.projected);
  ...
  return (
    <Panel aria-labelledby="trend-title">
      <PanelHeader
        ...
        description="Média dos indicadores percentuais · set/25–ago/26"
        action={
          <div role="tablist" aria-label="Modo de visualização" className="inline-flex rounded-lg bg-n-100 p-0.5">
            {([['history', 'Histórico'], ['scenario', 'Com cenário']] as const).map(([k, label]) => (
              <button
                ...
                onClick={() => setMode(k)}
                className={cn('inline-flex h-8 items-center gap-1.5 rounded-md px-3 text-xs font-semibold transition-colors', mode === k ? 'bg-n-0 text-n-900 shadow-[var(--shadow-1)]' : 'text-n-500 hover:text-n-800')}
              >
        ...
        <div className="h-[230px] w-full sm:h-[260px]">
          <ResponsiveContainer width="100%" height="100%" initialDimension={{ width: 640, height: 260 }}>
            <LineChart data={visibleData} margin={{ top: 12, right: 16, left: -14, bottom: 0 }}>
              ...
              {mode === 'scenario' && <ReferenceArea x1="Ago/26" x2="Nov/26" fill="var(--ink-50)" fillOpacity={0.7} strokeOpacity={0} label={{ value: 'Cenário Holt', position: 'insideTop', fontSize: 10, fill: 'var(--ink-700)' }} />}
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-t border-n-100 px-5 py-3 sm:px-6">
          ...
          {DOMAINS.map(d => {
            ...
            return (
              <button
                ...
                aria-pressed={!off}
                onClick={() => toggle(d)}
                className={cn('inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-xs font-medium transition-colors', off ? 'border-n-200 text-n-400 line-through' : 'border-n-200 bg-n-0 text-n-700 hover:bg-n-50', highlight === d && !off && 'border-n-400')}
              >
        ...
        <p className="text-[11px] text-n-500">
          {mode === 'scenario' ? 'Set–nov/26: suavização de Holt (α=0,45; β=0,25). Dados demonstrativos, sem validação preditiva.' : 'Contagens e tempos ficam fora da média percentual.'}
        </p>
      </div>
    </Panel>
  );
